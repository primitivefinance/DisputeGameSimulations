pub use optimism_portal_cannot_finalize_twice::*;
...
pub mod optimism_portal_cannot_finalize_twice {
    ...
    fn __abi() -> ::ethers::core::abi::Abi {
        ::ethers::core::abi::ethabi::Contract {
            ...
            functions: ::core::convert::From::from([
                ...
                (
                    ::std::borrow::ToOwned::to_owned("invariant_cannotFinalizeTwice"),
                    ::std::vec![
                        ::ethers::core::abi::ethabi::Function {
                            name: ::std::borrow::ToOwned::to_owned(
    ...
    impl<M: ::ethers::providers::Middleware> OptimismPortal_CannotFinalizeTwice<M> {
        ...
        pub fn new<T: Into<::ethers::core::types::Address>>(
            ...
            Self(
                ::ethers::contract::Contract::new(
                    address.into(),
                    OPTIMISMPORTAL_CANNOTFINALIZETWICE_ABI.clone(),
                    client,
                ),
            )
        }
        ...
        > {
            let factory = ::ethers::contract::ContractFactory::new(
                OPTIMISMPORTAL_CANNOTFINALIZETWICE_ABI.clone(),
                OPTIMISMPORTAL_CANNOTFINALIZETWICE_BYTECODE.clone().into(),
                client,
            );
        ...
        ///Calls the contract's `invariant_cannotFinalizeTwice` (0xd8f64bae) function
        pub fn invariant_cannot_finalize_twice(
            &self,
        ) -> ::ethers::contract::builders::ContractCall<M, ()> {
            self.0
                .method_hash([216, 246, 75, 174], ())
                .expect("method not found (this should never happen)")
        }
        ...
        pub fn events(
            ...
            ::std::sync::Arc<M>,
            M,
            OptimismPortal_CannotFinalizeTwiceEvents,
        > {
            self.0.event_with_filter(::core::default::Default::default())
        }
    }
    impl<M: ::ethers::providers::Middleware> From<::ethers::contract::Contract<M>>
    for OptimismPortal_CannotFinalizeTwice<M> {
    ...
    pub enum OptimismPortal_CannotFinalizeTwiceEvents {
    ...
    impl ::ethers::contract::EthLogDecode for OptimismPortal_CannotFinalizeTwiceEvents {
        fn decode_log(
            log: &::ethers::core::abi::RawLog,
        ) -> ::core::result::Result<Self, ::ethers::core::abi::Error> {
            if let Ok(decoded) = OutputProposedFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::OutputProposedFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = OutputsDeletedFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::OutputsDeletedFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = OwnershipTransferredFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::OwnershipTransferredFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = TransactionDepositedFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::TransactionDepositedFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = WithdrawalFinalizedFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::WithdrawalFinalizedFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = WithdrawalProvenFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::WithdrawalProvenFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogFilter::decode_log(log) {
                return Ok(OptimismPortal_CannotFinalizeTwiceEvents::LogFilter(decoded));
            }
            if let Ok(decoded) = LogAddressFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogAddressFilter(decoded),
                );
            }
            if let Ok(decoded) = LogArray1Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogArray1Filter(decoded),
                );
            }
            if let Ok(decoded) = LogArray2Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogArray2Filter(decoded),
                );
            }
            if let Ok(decoded) = LogArray3Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogArray3Filter(decoded),
                );
            }
            if let Ok(decoded) = LogBytesFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogBytesFilter(decoded),
                );
            }
            if let Ok(decoded) = LogBytes32Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogBytes32Filter(decoded),
                );
            }
            if let Ok(decoded) = LogIntFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogIntFilter(decoded),
                );
            }
            if let Ok(decoded) = LogNamedAddressFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogNamedAddressFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogNamedArray1Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogNamedArray1Filter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogNamedArray2Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogNamedArray2Filter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogNamedArray3Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogNamedArray3Filter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogNamedBytesFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogNamedBytesFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogNamedBytes32Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogNamedBytes32Filter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogNamedDecimalIntFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogNamedDecimalIntFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogNamedDecimalUintFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogNamedDecimalUintFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogNamedIntFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogNamedIntFilter(decoded),
                );
            }
            if let Ok(decoded) = LogNamedStringFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogNamedStringFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogNamedUintFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogNamedUintFilter(decoded),
                );
            }
            if let Ok(decoded) = LogStringFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogStringFilter(decoded),
                );
            }
            if let Ok(decoded) = LogUintFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotFinalizeTwiceEvents::LogUintFilter(decoded),
                );
            }
            if let Ok(decoded) = LogsFilter::decode_log(log) {
                return Ok(OptimismPortal_CannotFinalizeTwiceEvents::LogsFilter(decoded));
            }
            Err(::ethers::core::abi::Error::InvalidData)
        }
    }
    impl ::core::fmt::Display for OptimismPortal_CannotFinalizeTwiceEvents {
        fn fmt(&self, f: &mut ::core::fmt::Formatter<'_>) -> ::core::fmt::Result {
            match self {
    ...
    for OptimismPortal_CannotFinalizeTwiceEvents {
    ...
    impl ::core::convert::From<LogFilter> for OptimismPortal_CannotFinalizeTwiceEvents {
    ...
    impl ::core::convert::From<LogsFilter> for OptimismPortal_CannotFinalizeTwiceEvents {
    ...
    ///Container type for all input parameters for the `invariant_cannotFinalizeTwice` function with signature `invariant_cannotFinalizeTwice()` and selector `0xd8f64bae`
    #[derive(
        Clone,
        ::ethers::contract::EthCall,
        ::ethers::contract::EthDisplay,
    ...
    #[ethcall(
        ...
        abi = "invariant_cannotFinalizeTwice()"
    )]
    pub struct InvariantCannotFinalizeTwiceCall;
    ...
    #[derive(Clone, ::ethers::contract::EthAbiType, Debug, PartialEq, Eq, Hash)]
    pub enum OptimismPortal_CannotFinalizeTwiceCalls {
        IsTest(IsTestCall),
        ExcludeArtifacts(ExcludeArtifactsCall),
        ExcludeContracts(ExcludeContractsCall),
        ExcludeSenders(ExcludeSendersCall),
        Failed(FailedCall),
        InvariantCannotFinalizeTwice(InvariantCannotFinalizeTwiceCall),
    ...
    impl ::ethers::core::abi::AbiDecode for OptimismPortal_CannotFinalizeTwiceCalls {
        fn decode(
            data: impl AsRef<[u8]>,
            ...
            if let Ok(decoded) = <InvariantCannotFinalizeTwiceCall as ::ethers::core::abi::AbiDecode>::decode(
                data,
            ) {
                return Ok(Self::InvariantCannotFinalizeTwice(decoded));
            }
    ...
    impl ::ethers::core::abi::AbiEncode for OptimismPortal_CannotFinalizeTwiceCalls {
        fn encode(self) -> Vec<u8> {
            match self {
                ...
                Self::InvariantCannotFinalizeTwice(element) => {
                    ::ethers::core::abi::AbiEncode::encode(element)
                }
    ...
    impl ::core::fmt::Display for OptimismPortal_CannotFinalizeTwiceCalls {
        fn fmt(&self, f: &mut ::core::fmt::Formatter<'_>) -> ::core::fmt::Result {
            match self {
    ...
    impl ::core::convert::From<IsTestCall> for OptimismPortal_CannotFinalizeTwiceCalls {
        fn from(value: IsTestCall) -> Self {
            Self::IsTest(value)
        }
    }
    impl ::core::convert::From<ExcludeArtifactsCall>
    for OptimismPortal_CannotFinalizeTwiceCalls {
    ...
    impl ::core::convert::From<FailedCall> for OptimismPortal_CannotFinalizeTwiceCalls {
        fn from(value: FailedCall) -> Self {
            Self::Failed(value)
        }
    }
    impl ::core::convert::From<InvariantCannotFinalizeTwiceCall>
    for OptimismPortal_CannotFinalizeTwiceCalls {
        fn from(value: InvariantCannotFinalizeTwiceCall) -> Self {
            Self::InvariantCannotFinalizeTwice(value)
        }
    }
    ...
    impl ::core::convert::From<SetUpCall> for OptimismPortal_CannotFinalizeTwiceCalls {
