pub use optimism_portal_cannot_time_travel::*;
...
pub mod optimism_portal_cannot_time_travel {
    ...
    impl<M: ::ethers::providers::Middleware> OptimismPortal_CannotTimeTravel<M> {
        ...
        pub fn new<T: Into<::ethers::core::types::Address>>(
            ...
            Self(
                ::ethers::contract::Contract::new(
                    address.into(),
                    OPTIMISMPORTAL_CANNOTTIMETRAVEL_ABI.clone(),
                    client,
                ),
            )
        }
        ...
        > {
            let factory = ::ethers::contract::ContractFactory::new(
                OPTIMISMPORTAL_CANNOTTIMETRAVEL_ABI.clone(),
                OPTIMISMPORTAL_CANNOTTIMETRAVEL_BYTECODE.clone().into(),
                client,
            );
        ...
        ///Calls the contract's `invariant_cannotFinalizeBeforePeriodHasPassed` (0xfbcdff8d) function
        pub fn invariant_cannot_finalize_before_period_has_passed(
            &self,
        ) -> ::ethers::contract::builders::ContractCall<M, ()> {
            self.0
                .method_hash([251, 205, 255, 141], ())
                .expect("method not found (this should never happen)")
        }
        ...
        pub fn events(
            ...
            ::std::sync::Arc<M>,
            M,
            OptimismPortal_CannotTimeTravelEvents,
        > {
            self.0.event_with_filter(::core::default::Default::default())
        }
    }
    impl<M: ::ethers::providers::Middleware> From<::ethers::contract::Contract<M>>
    for OptimismPortal_CannotTimeTravel<M> {
    ...
    pub enum OptimismPortal_CannotTimeTravelEvents {
    ...
    impl ::ethers::contract::EthLogDecode for OptimismPortal_CannotTimeTravelEvents {
        fn decode_log(
            log: &::ethers::core::abi::RawLog,
        ) -> ::core::result::Result<Self, ::ethers::core::abi::Error> {
            if let Ok(decoded) = OutputProposedFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::OutputProposedFilter(decoded),
                );
            }
            if let Ok(decoded) = OutputsDeletedFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::OutputsDeletedFilter(decoded),
                );
            }
            if let Ok(decoded) = OwnershipTransferredFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::OwnershipTransferredFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = TransactionDepositedFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::TransactionDepositedFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = WithdrawalFinalizedFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::WithdrawalFinalizedFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = WithdrawalProvenFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::WithdrawalProvenFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogFilter::decode_log(log) {
                return Ok(OptimismPortal_CannotTimeTravelEvents::LogFilter(decoded));
            }
            if let Ok(decoded) = LogAddressFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogAddressFilter(decoded),
                );
            }
            if let Ok(decoded) = LogArray1Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogArray1Filter(decoded),
                );
            }
            if let Ok(decoded) = LogArray2Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogArray2Filter(decoded),
                );
            }
            if let Ok(decoded) = LogArray3Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogArray3Filter(decoded),
                );
            }
            if let Ok(decoded) = LogBytesFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogBytesFilter(decoded),
                );
            }
            if let Ok(decoded) = LogBytes32Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogBytes32Filter(decoded),
                );
            }
            if let Ok(decoded) = LogIntFilter::decode_log(log) {
                return Ok(OptimismPortal_CannotTimeTravelEvents::LogIntFilter(decoded));
            }
            if let Ok(decoded) = LogNamedAddressFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogNamedAddressFilter(decoded),
                );
            }
            if let Ok(decoded) = LogNamedArray1Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogNamedArray1Filter(decoded),
                );
            }
            if let Ok(decoded) = LogNamedArray2Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogNamedArray2Filter(decoded),
                );
            }
            if let Ok(decoded) = LogNamedArray3Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogNamedArray3Filter(decoded),
                );
            }
            if let Ok(decoded) = LogNamedBytesFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogNamedBytesFilter(decoded),
                );
            }
            if let Ok(decoded) = LogNamedBytes32Filter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogNamedBytes32Filter(decoded),
                );
            }
            if let Ok(decoded) = LogNamedDecimalIntFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogNamedDecimalIntFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogNamedDecimalUintFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogNamedDecimalUintFilter(
                        decoded,
                    ),
                );
            }
            if let Ok(decoded) = LogNamedIntFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogNamedIntFilter(decoded),
                );
            }
            if let Ok(decoded) = LogNamedStringFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogNamedStringFilter(decoded),
                );
            }
            if let Ok(decoded) = LogNamedUintFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogNamedUintFilter(decoded),
                );
            }
            if let Ok(decoded) = LogStringFilter::decode_log(log) {
                return Ok(
                    OptimismPortal_CannotTimeTravelEvents::LogStringFilter(decoded),
                );
            }
            if let Ok(decoded) = LogUintFilter::decode_log(log) {
                return Ok(OptimismPortal_CannotTimeTravelEvents::LogUintFilter(decoded));
            }
            if let Ok(decoded) = LogsFilter::decode_log(log) {
                return Ok(OptimismPortal_CannotTimeTravelEvents::LogsFilter(decoded));
            }
            Err(::ethers::core::abi::Error::InvalidData)
        }
    }
    impl ::core::fmt::Display for OptimismPortal_CannotTimeTravelEvents {
        fn fmt(&self, f: &mut ::core::fmt::Formatter<'_>) -> ::core::fmt::Result {
            match self {
    ...
    for OptimismPortal_CannotTimeTravelEvents {
    ...
    impl ::core::convert::From<LogFilter> for OptimismPortal_CannotTimeTravelEvents {
    ...
    impl ::core::convert::From<LogIntFilter> for OptimismPortal_CannotTimeTravelEvents {
    ...
    impl ::core::convert::From<LogUintFilter> for OptimismPortal_CannotTimeTravelEvents {
        fn from(value: LogUintFilter) -> Self {
            Self::LogUintFilter(value)
        }
    }
    impl ::core::convert::From<LogsFilter> for OptimismPortal_CannotTimeTravelEvents {
    ...
    ///Container type for all input parameters for the `invariant_cannotFinalizeBeforePeriodHasPassed` function with signature `invariant_cannotFinalizeBeforePeriodHasPassed()` and selector `0xfbcdff8d`
    #[derive(
        Clone,
        ::ethers::contract::EthCall,
        ::ethers::contract::EthDisplay,
    ...
    #[ethcall(
        ...
        abi = "invariant_cannotFinalizeBeforePeriodHasPassed()"
    )]
    pub struct InvariantCannotFinalizeBeforePeriodHasPassedCall;
    ...
    #[derive(Clone, ::ethers::contract::EthAbiType, Debug, PartialEq, Eq, Hash)]
    pub enum OptimismPortal_CannotTimeTravelCalls {
        IsTest(IsTestCall),
        ExcludeArtifacts(ExcludeArtifactsCall),
        ExcludeContracts(ExcludeContractsCall),
        ExcludeSenders(ExcludeSendersCall),
        Failed(FailedCall),
        InvariantCannotFinalizeBeforePeriodHasPassed(
            InvariantCannotFinalizeBeforePeriodHasPassedCall,
        ),
    ...
    impl ::ethers::core::abi::AbiDecode for OptimismPortal_CannotTimeTravelCalls {
        fn decode(
            data: impl AsRef<[u8]>,
            ...
            if let Ok(decoded) = <InvariantCannotFinalizeBeforePeriodHasPassedCall as ::ethers::core::abi::AbiDecode>::decode(
                data,
            ) {
                return Ok(Self::InvariantCannotFinalizeBeforePeriodHasPassed(decoded));
            }
    ...
    impl ::ethers::core::abi::AbiEncode for OptimismPortal_CannotTimeTravelCalls {
        fn encode(self) -> Vec<u8> {
            match self {
                ...
                Self::InvariantCannotFinalizeBeforePeriodHasPassed(element) => {
                    ::ethers::core::abi::AbiEncode::encode(element)
                }
    ...
    impl ::core::fmt::Display for OptimismPortal_CannotTimeTravelCalls {
        fn fmt(&self, f: &mut ::core::fmt::Formatter<'_>) -> ::core::fmt::Result {
            match self {
    ...
    impl ::core::convert::From<IsTestCall> for OptimismPortal_CannotTimeTravelCalls {
        fn from(value: IsTestCall) -> Self {
            Self::IsTest(value)
        }
    }
    impl ::core::convert::From<ExcludeArtifactsCall>
    for OptimismPortal_CannotTimeTravelCalls {
    ...
    impl ::core::convert::From<FailedCall> for OptimismPortal_CannotTimeTravelCalls {
        fn from(value: FailedCall) -> Self {
            Self::Failed(value)
        }
    }
    impl ::core::convert::From<InvariantCannotFinalizeBeforePeriodHasPassedCall>
    for OptimismPortal_CannotTimeTravelCalls {
        fn from(value: InvariantCannotFinalizeBeforePeriodHasPassedCall) -> Self {
            Self::InvariantCannotFinalizeBeforePeriodHasPassed(value)
        }
    }
    ...
    impl ::core::convert::From<SetUpCall> for OptimismPortal_CannotTimeTravelCalls {
